<template>
	<div class="app" @click="flag = false">
		<div class="header" :class="SearchFlag === true ? 'wide' : ''">
			...
			<div class="header-menu">
				<router-link
					class="menu-link"
					:class="[tabSwith === index ? 'is-active' : '', (index + 1) % 2 === 0 ? 'notify' : '']"
					:to="item.path"
					@click="tabSwith = index"
					v-for="(item, index) in tabSwithList"
				>
					{{ item.title }}
				</router-link>
			</div>
			...
			<div class="left-side" v-if="status && sideShow">
				<div class="side-wrapper">
					...
					<div class="side-menu">
						...
						<router-link @click="sideShow = false" to="/apps/games">
							<svg
								t="1669266744050"
								class="icon"
						...
						<router-link @click="sideShow = false" to="/apps/echartmap">
							...
							>
								<path
									d="M87.45984 588.91264 827.262976 186.5728 491.702272 958.961664 416.700416 638.511104Z"
									p-id="8700"
								></path>
							</svg>
							Echart Map
						</router-link>
						<router-link @click="sideShow = false" to="/apps/setup">
							<svg
								t="1662014382031"
								fill="currentColor"
								viewBox="0 0 1024 1024"
	...
	<a-player v-if="store.AplayFlag" :key="timer" :fixed="store.AplayFixedMode"></a-player>
</template>
